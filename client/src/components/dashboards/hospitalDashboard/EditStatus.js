...
class Esd extends Component {
  ...
  onClick(e){
    axios({
      url:`/api/doctor/delStatus-${this.props.id}`,
      method:'delete',
      headers:{
  ...
  onSubmit(e){
    ...
    if(!this.state.token)this.setState({isToken:true});
      ...
      axios({
        url:`/api/doctor/editStatus-${this.props.id}`,
        method:'post',
        headers:{
          Authorization:localStorage.getItem('hospital')
        },
  ...
  render(){
    return(
      ...
      onSubmit={this.onSubmit} style={{"background":"#fafafa"}}>
      <div className="form-group my-5">
      <label htmlFor="exampleInputPassword1">Select Status</label>
      <br/>
      <select name='status' id='exampleInputPassword1'
      onChange={this.onChange} className='w-100'>
...
<input type="text"
  className={this.state.isToken?"form-control is-invalid":"form-control"}
  name="token" id="exampleInputEmail1" aria-describedby="emailHelp" value={this.state.token} onChange={this.onChange}/>
<p className='text-danger mt-3'>
{this.state.isToken?'Please enter the token number first.':''}
</p>
</div>
<button type="submit" className="my-5 btn estu rounded-0 mx-auto d-block text-uppercase">
Update <i className="fa fa-check ml-1" aria-hidden="true"></i></button>
</form>
<button onClick={this.onClick}
type="submit" className="my-5 btn btn-block btn-outline-danger rounded-0 mx-auto d-block text-uppercase">
remove doctor <i className="fa fa-times ml-1" aria-hidden="true"></i></button>
      </div>
      </div>
    );
  }
